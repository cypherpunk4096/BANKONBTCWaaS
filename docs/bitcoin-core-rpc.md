# Bitcoin Core v31 — RPC Command Reference (local)

Captured from `bitcoin-cli help` on this node. For full detail on any command:
`bitcoin-cli help <command>` or use the Console → RPC Console / Reference tab.
BANKON's Console exposes the **read-only** subset (see bankon-console/server.mjs).
