// registry.mjs — BANKON.ETH (EVM) WaaS wallet registry (file-backed, PUBLIC metadata only).
// Twin of bankon-waas/registry.mjs. Maps EVM address → { address, chainId, owner, label, createdAt }.
// NEVER stores private material (mnemonic / private key) — defended below.
import { readFileSync, writeFileSync, renameSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';

const __dir = dirname(fileURLToPath(import.meta.url));
const FILE = process.env.BANKON_ETH_REGISTRY || join(__dir, 'registry.json');
const PRIVATE = ['mnemonic', 'privatekey', 'privkey', 'seed', 'passphrase', 'keystore'];
